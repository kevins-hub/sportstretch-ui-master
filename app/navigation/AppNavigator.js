...
function AppNavigator({user}) {
    ...
    const pastBookingComponent = user.role === 'therapist' ? TherapistPastBooking : AthletePastBooking;
    const upcomingBookingComponent = user.role === 'therapist' ? TherapistUpcomingBooking : AthleteUpcomingBooking;
    ...
    return (
        <>  
            ...
            <appTabs.Navigator initialRouteName="profile" screenOptions={ ({route}) =>({
                ...
                {user.role === 'athlete' && <appTabs.Screen name="Make an Appointment" component={AthleteBookNow} options={{ tabBarLabel: 'Search'}}></appTabs.Screen>}
                <appTabs.Screen name="Past Appointments" component={pastBookingComponent} options={{ tabBarLabel: 'History' }}></appTabs.Screen>
                <appTabs.Screen name="Upcoming Appointments" component={upcomingBookingComponent} options={{ tabBarLabel: 'Upcoming' }}></appTabs.Screen>
                <appTabs.Screen name="Profile" component={ProfileSettings} initialParams={{user: user}} options={{ tabBarLabel: 'Profile'}}></appTabs.Screen>
            </appTabs.Navigator>
        </>
    );
}
