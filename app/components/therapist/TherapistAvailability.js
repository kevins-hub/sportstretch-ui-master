...
function TherapistAvailability(props) {
  ...
  const setAvailabilityStatus = () => {
    return isEnabled ? (
      <View style={styles.availabilityContainer}>
        <Image
          source={require("../../assets/available.jpeg")}
  ...
  const [isAvailable, setIsAvailable] = useState(setAvailabilityStatus);

  const toggleSwitch = async () => {
    let avail_status = await therapistsApi.setAvailability(
      user.userObj.therapist_id,
      { availability_status: !isEnabled }
    );
    avail_status.data.availability_status == !isEnabled
      ? setIsEnabled((previousState) => !previousState)
      : Alert.alert("Error while updating availability. Please try again.");
  };

  useEffect(() => {
    //runs when page re-renders or songs change but not for age
    setIsAvailable(setAvailabilityStatus);
  }, [isEnabled]);
